...
def launch_instance(compute_kwargs={}, oci_kwargs={}, subnet_kwargs={}, vcn_kwargs={}):
    ...
    available_images = list_entities(
        compute_client, "list_images", compartment_id=oci_kwargs["compartment_id"]
    )

    selected_image = None
    for image in available_images:
        if (
            image.operating_system == compute_kwargs["os"]
            and image.operating_system_version == compute_kwargs["os_version"]
        ):
            selected_image = image

    if not selected_image:
        print(
            "Failed to find the selected image: {}-{}".format(
                compute_kwargs["os"], compute_kwargs["os_version"]
            )
        )
        return False

    instance_source_via_image_details = InstanceSourceViaImageDetails(
    ...
    for shape in available_shapes:
        if shape.shape == compute_kwargs["target_shape"]:
            selected_shape = shape

    if not selected_shape:
        print(
            "Failed to find the selected shape: {}".format(
                compute_kwargs["target_shape"]
            )
        )
        return False
